...
static struct Chain *insert(struct List *self, void *element) {
    ...
    struct Chain *addition = my_malloc(sys_heap, sizeof(struct Chain));
...
void ListInit(struct List *self) {
    extern struct Heap *sys_heap;
    self->head = my_malloc(sys_heap, sizeof(struct Chain));
    self->tail = my_malloc(sys_heap, sizeof(struct Chain));
    self->head->back = NULL;
